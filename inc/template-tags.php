 ...
 */
  ...
  */
 // function replace_content($s){
 //  // replace all instances of $s with in the content with parameter
 //   $content = get_the_content();
 //   echo str_replace("WordPress", $s, $content);
 // }
  function replace_content($find,$replace){
  // replace all instances of $s with in the content with parameter
   $content = get_the_content();
   echo str_replace($find,$replace, $content);
 }
 function red_starter_title($purple=false){
  if ($purple = "echo"){
   echo strtoupper('<h1>'.get_the_title().'</h1>');
  } else{
